...
> **1차(round_01) 6/16(37.5%) → 11차(round_11) 16/16(100%).** 가장 크게 기여한 변경은 round_04의 `linkify_places` 수정이다 — 명소 이름에 링크를 거는 위치를 "텍스트 처음 등장하는 자리"에서 "소제목·굵은 라벨 자리"로 정확히 고치면서 통과율이 11/16에서 16/16으로 뛰었다.

`evaluation/run_eval.py`로 16개 평가 케이스를 실행할 때마다 라운드가 하나씩 쌓인다
(`evaluation/results/round_NN/report.md`). 지금까지 **총 11회** 실행했고, 그 중 7번은 문제를 찾거나 로직을 개선해 코드를 고친 뒤 재실행한 경우다.
...
| round_10 | 16/16 | "강원도 여주시"(실존하지 않는 시/도+시/군 조합)가 무관한 가게 이름("강원도자연애생산삼", 실제 주소는 경기 여주시)에 걸려 존재하는 지역으로 잘못 판정되는 버그 발견 — 그 결과 조회 결과 표에 쌓이고 임직원 메일까지 노출됐다. 1차 수정: 시/도 이름으로 시작하는 질의는 검색 결과 주소에 그 시/도 축약형이 실제로 포함되는지 추가 검증. 회귀 없음 확인. |
| round_11 | 16/16 | 담당자가 명소 이름을 직접 입력하는 경우는 없다는 걸 다시 확인해, round_10의 보정 로직 대신 지역 검증 자체를 카카오 키워드(장소) 검색에서 **행정구역 주소 검색**으로 되돌려 더 근본적으로 해결(랜드마크 매칭이 필요 없으므로). 회귀 없음 확인. |
